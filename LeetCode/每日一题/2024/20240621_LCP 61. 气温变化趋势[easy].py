#!/usr/bin/env python
# -*- coding: UTF-8 -*-
# @File    ：20240621_LCP 61. 气温变化趋势[easy].py
# @Author  ：Lin
# @Date    ：2024/6/21 9:55
...
class Solution:
    def temperatureTrend(self, temperatureA: List[int], temperatureB: List[int]) -> int:
        ...
        res, current = 0, 0
        for i in range(1, n):
            if any([temperatureA[i] - temperatureA[i-1] > 0 and temperatureB[i] - temperatureB[i-1] > 0,
            temperatureA[i] - temperatureA[i-1] == 0 and temperatureB[i] - temperatureB[i-1] == 0,
            temperatureA[i] - temperatureA[i-1] < 0 and temperatureB[i] - temperatureB[i-1] < 0]):
                current += 1
                res = max(res, current)
            else:
