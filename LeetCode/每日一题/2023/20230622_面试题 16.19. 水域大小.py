#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @File  : 20230622_面试题 16.19. 水域大小.py
# @Author: Lin
    ...
    def pondSizes(self, land: List[List[int]]) -> List[int]:
        ...
        def bfs(x, y):
            ...
            while deq:
                ...
                for r in range(-1, 2):
                    for c in range(-1, 2):
                        if 0 <= dx + r < m and 0 <= dy + c < n and land[dx+r][dy+c] == 0:
                            deq.append((dx+r, dy+c))
                            land[dx+r][dy+c] = -1
            return res
        ...
        for i in range(m):
            for j in range(n):
                if land[i][j] == 0:
                    ans.append(bfs(i, j))
                    # bisect.insort_left(ans, bfs(i, j))
        ans.sort()
        return ans
