...
# @Author  : Lin
# @File    : 20250713_2410. 运动员和训练师的最大匹配数[mid].py
...
class Solution:
    def matchPlayersAndTrainers(self, players: List[int], trainers: List[int]) -> int:
        players.sort()
        trainers.sort()
        n, right = len(trainers), 0
        for left in range(len(players)):
            while right < n and players[left] > trainers[right]:
                right += 1
            if right == n:
                return left
            right += 1
...
class Solution:
    def matchPlayersAndTrainers(self, players: List[int], trainers: List[int]) -> int:
        players.sort()
        trainers.sort()
        m, left = len(players), 0
        for t in trainers:
            if left < m and players[left] <= t:
                left += 1
        return left
